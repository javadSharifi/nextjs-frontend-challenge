...
const GenericTable = <T extends { id: string | number }>({
  ...
  return (
    <Box
      ...
      shadow="sm"
    >
      <Table.Root size="md" variant="line" interactive color="fg">
        <Table.Header bg={{ base: 'gray.100', _dark: 'whiteAlpha.100' }}>
          <Table.Row borderBottom="2px solid" borderColor="border.muted">
            {columns.map((col) => (
              <Table.ColumnHeader key={col.header} color="fg" fontWeight="800" py="4">
                {col.header}
              </Table.ColumnHeader>
            ))}
            {showActions && (
              <Table.ColumnHeader w="120px" color="fg" fontWeight="800" py="4">
                {t('actions')}
              </Table.ColumnHeader>
            )}
          </Table.Row>
        ...
        <Table.Body>
          {isLoading
            ...
            : data.map((item) => (
                ...
                >
                  {columns.map((col) => (
                    <Table.Cell key={col.header} py="4" fontWeight="medium" color="fg">
                      {col.render(item)}
                    </Table.Cell>
                  ))}
