  ...
  return useQuery({
    ...
    queryFn: async () => {
      const [totalRes, adminsRes, moderatorsRes] = await Promise.all([
        apiClient.get<UsersResponse>('/users', { params: { limit: 0, select: 'id' } }),
        apiClient.get<UsersResponse>('/users/filter', {
          params: { key: 'role', value: 'admin', limit: 0 },
        }),
        apiClient.get<UsersResponse>('/users/filter', {
          params: { key: 'role', value: 'moderator', limit: 0 },
        }),
      ]);
