...
export const useGameParams = () => {
  ...
  const [ordering, setOrdering] = useQueryState('ordering', parseAsStringEnum(['-rating','-released','-metacritic','name','-name','-added']).withDefault('-rating'));
  const [metacritic_min, setMetacriticMin] = useQueryState('metacritic_min', parseAsInteger.withDefault(0));
  const [metacritic_max, setMetacriticMax] = useQueryState('metacritic_max', parseAsInteger.withDefault(100));
  ...
  return {
    ...
    metacritic_min, setMetacriticMin,
    metacritic_max, setMetacriticMax,
    page, setPage,
    tags, setTags,
    view, setView
  };
