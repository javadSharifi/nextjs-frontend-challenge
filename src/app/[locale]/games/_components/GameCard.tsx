...
const GameCard = ({ game, view = 'grid' }: IGameCardProps) => {
  ...
  if (view === 'list') {
    return (
      <GameCardLink href={`/games/${game.slug}`}>
        <div className="card card-side bg-base-100/50 hover:bg-base-100/80 cursor-pointer gap-4 border border-white/5 p-4 shadow-xl transition-all">
          ...
          <div className="flex flex-1 flex-col justify-center gap-1">
            ...
            <p className="text-xs opacity-70">{game.genres?.map((g) => g.name).join(', ')}</p>
          </div>
  ...
  return (
    <GameCardLink href={`/games/${game.slug}`}>
      <div className="card bg-base-100 group border-base-content/5 hover:border-primary/50 hover:shadow-primary/20 cursor-pointer overflow-hidden border transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl">
        <figure className="relative aspect-[16/9] overflow-hidden shadow-lg">
          ...
          <div className="absolute bottom-2 left-2 flex gap-1">
            {game.platforms?.slice(0, 3).map((p) => (
              <span
                key={p.platform.id}
                className="badge badge-ghost badge-xs border-none bg-black/60 text-white/70 backdrop-blur-sm"
              >
        ...
        <div className="card-body gap-1 p-4">
          <div className="flex items-start justify-between gap-2">
            <h3 className="card-title group-hover:text-primary line-clamp-1 text-base transition-colors">
              {game.name}
            </h3>
            {game.metacritic && (
          ...
          <p className="text-sm opacity-60">{game.genres?.map((g) => g.name).join(' · ')}</p>
        </div>
      </div>
    </GameCardLink>
  );
