      ...
      <div className="glass-card group cursor-pointer overflow-hidden rounded-xl">
        ...
        <div className="relative aspect-[16/9] overflow-hidden">
          ...
          <div className="absolute bottom-2 left-2 flex gap-1">
            {game.platforms?.slice(0, 3).map((p) => (
              <span
                key={p.platform.id}
                className="rounded bg-black/60 px-1.5 py-0.5 text-xs text-text-secondary backdrop-blur-sm"
              >
        ...
        <div className="p-4">
          <div className="mb-1 flex items-start justify-between gap-2">
            <h3 className="font-display font-bold text-white line-clamp-1 group-hover:text-primary transition-colors">
              {game.name}
            </h3>
            {game.metacritic && (
