  ...
  return (
    <div className="glass-card rounded-xl p-5 space-y-4">
      ...
      {infoRows.map(({ label, value, icon: Icon }) => (
        <div key={label} className="flex items-center justify-between border-b border-border-subtle pb-3">
      ...
      {game.tags && game.tags.length > 0 && (
        <div>
          <p className="mb-2 text-xs text-text-muted uppercase tracking-wider">{t('detail.info.tags')}</p>
      ...
      {game.website && (
        <a
          ...
          className="flex items-center gap-2 rounded-lg border border-border-accent px-4 py-2 text-sm text-primary hover:bg-primary-muted transition-colors w-full justify-center"
        >
          <Globe size={14} />
