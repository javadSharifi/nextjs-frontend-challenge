...
const ScreenshotModal = ({
  ...
  return createPortal(
    <div
      className="modal modal-open z-[9999] bg-black/80 backdrop-blur-md p-4"
      onMouseDown={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div
        className="modal-box relative w-full max-w-3xl overflow-hidden rounded-2xl border border-white/10 bg-base-300 p-0 shadow-2xl"
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {screenshots.map((s, i) => (
            <button
              ...
              className="relative h-24 w-40 shrink-0 overflow-hidden rounded-lg border border-white/10 transition-all duration-200 hover:scale-105 hover:border-primary"
            >
              <Image
                src={s.image}
                alt={`${gameName} ${i + 1}`}
